...
vector<pair<vector<ImuConstPtr>, ImgConstPtr>> System::getMeasurements()
{
    ...
    while (true)
    {
        ...
        //第一帧图像到来前已有IMU数据，继续等待，等待的IMU数据个数存储在sum_of_wait中，一直等到新的IMU“夹住”图像
    ...
    while (bStart_backend)
    {
        ...
        for (auto &measurement : measurements)
        {
            ...
            for (auto &imu_msg : measurement.first)
            {
                ...
                if (t <= img_t)//理论上至少有一个IMU的时间戳大于img的时间戳
                {
                    if (current_time < 0)
                        current_time = t;
            ...
            if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR)
            {
                ...
                ofs_pose.setf(ios::fixed, ios::floatfield);
                ofs_pose<<fixed<<dStamp<<" ";
                ofs_pose.precision(7);
                ofs_pose<<p_wi[0]<<" " <<p_wi[1]<<" "<<p_wi[2]<<" "<<q_wi.x()<<
