...
void Estimator::processIMU(double dt, const Vector3d &linear_acceleration, const Vector3d &angular_velocity)
{
    ...
    if (frame_count != 0)
    {
        ...
        int j = frame_count;
        //中值积分法
        Vector3d un_acc_0 = Rs[j] * (acc_0 - Bas[j]) - g;//Rs即是Rib，从体坐标系转到世界坐标系
...
void Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, double header)
{
    ...
    else
    {
        ...
        if (failureDetection())
        {
            ...
            failure_occur = 1;
            clearState();//一旦检测到失败，franme_count从0开始
            setParameter();
...
bool Estimator::initialStructure()
{
    ...
    for (auto &it_per_id : f_manager.feature)
    {
        ...
        for (auto &it_per_frame : it_per_id.feature_per_frame)
        {
            ...
            tmp_feature.observation.push_back(make_pair(imu_j, Eigen::Vector2d{pts_j.x(), pts_j.y()}));//归一化坐标xy
        }
    ...
    Vector3d relative_T;
    int l;
    //对极约束求Rwc，twc,并从窗口中找出先前的共视路标点多的且视差足够的一帧为l
...
void Estimator::slideWindowOld()
{
    ...
    if (shift_depth)
    {
        Matrix3d R0, R1;
        Vector3d P0, P1;
        R0 = back_R0 * ric[0];//被marg掉的，旧的
        R1 = Rs[0] * ric[0];//新窗口第一帧
