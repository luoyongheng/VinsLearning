...
{
    std::random_device rd;//引擎
    std::default_random_engine generator_(rd());//生成
    std::normal_distribution<double> noise(0.0, 1.0);//均值与方差
...
MotionData IMU::MotionModel(double t)
{
    ...
    float K = M_PI/10;    // 20 * K = 2pi 　　由于我们采取的是时间周期是20s, 系数K控制yaw正好旋转一圈，运动一周
...
//        std::cout<<"dtheta"<<dtheta_half<<std::endl;
//        dq.w() = 1;
//        dq.x() = dtheta_half.x();
//        dq.y() = dtheta_half.y();
//        dq.z() = dtheta_half.z();
//
//        Eigen::Vector3d acc_w = Qwb * (imupose.imu_acc) + gw;  // aw = Rwb * acc_body + gw
//        Qwb = Qwb * dq;
//        Vw = Vw + acc_w * dt;
//        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        ...
        Eigen::Vector3d dtheta_half = (imudata[i-1].imu_gyro + imudata[i].imu_gyro)*dt/4.0;
        ...
        Eigen::Quaterniond QwbBefore = Qwb;
        Qwb = Qwb * dq;
        Eigen::Vector3d acc_w = (QwbBefore*(imudata[i-1].imu_acc) + Qwb*(imudata[i].imu_acc) + 2*gw)/2.0;
        Vw = Vw + acc_w * dt;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
