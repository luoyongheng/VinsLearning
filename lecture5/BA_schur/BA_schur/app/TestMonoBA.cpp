...
int main() {
    ...
    Eigen::Quaterniond qic(1, 0, 0, 0);//从相机到imu的旋转矩阵，假设为单位阵
    Eigen::Vector3d tic(0, 0, 0);//从相机到imu的平移，假设为0
...
//            vertexCam->SetFixed();
    ...
    for (size_t i = 0; i < points.size(); ++i) {
        ...
        double inverse_depth = 1. / (Pc.z() + noise);//只在深度信息上加了噪声
//        double inverse_depth = 1. / Pc.z();
        ...
        for (size_t j = 1; j < cameras.size(); ++j) {
            ...
            edge_vertex.push_back(vertexCams_vec[j]);//多态
