...
void Problem::MakeHessian() {
    ...
    for (auto &edge: edges_) {
        ...
        assert(jacobians.size() == verticies.size());//都是3
        for (size_t i = 0; i < verticies.size(); ++i) {
            auto v_i = verticies[i];
            if (v_i->IsFixed()) continue;    // Hessian 里不需要添加它的信息，也就是它的雅克比为 0

            auto jacobian_i = jacobians[i];
            ulong index_i = v_i->OrderingId();//雅克比矩阵块应该出现的位置
            ulong dim_i = v_i->LocalDimension();//对于逆深度来说是1维，对于两个相机位姿来说是6维
...
//        delta_x_ = PCGSolver(H, b_, H.rows() * 2);
        ...
        for (auto landmarkVertex : idx_landmark_vertices_) {
            ...
            Hmm_inv.block(idx, idx, size, size) = Hmm.block(idx, idx, size, size).inverse();//小矩阵块求逆
        }
...
void Problem::UpdateStates() {
    for (auto vertex: verticies_) {
        ...
        VecX delta = delta_x_.segment(idx, dim);//delta_x_是总的优化变量，维数是顶点1维数×顶点1个数+顶点2维数×顶点2个数+...
        vertex.second->Plus(delta);//vertex.second.parameters_是一个顶点
    }
    if (err_prior_.rows() > 0) {
        b_prior_ -= H_prior_ * delta_x_.head(ordering_poses_);   // update the error_prior
